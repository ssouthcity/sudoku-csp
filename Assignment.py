...
# Generic T represents the type of values in the CSP
T = TypeVar("T")
...
class CSP(Generic[T]):
    ...
    constraints: dict[str, dict[str, list[tuple[T, T]]]]
    ...
    def add_variable(self, name: str, domain: list[T]):
    ...
    def add_constraint_one_way(
        self, i: str, j: str, filter_function: Callable[[T, T], bool]
    ):
    ...
    def add_all_different_constraint(self, var_list: list[str]):
    ...
    def backtrack(self, assignment: dict[str, list[T]]) -> dict[str, list[T]] | None:
        ...
        for value in assignment[var]:
            ...
            if not self.inference(assignment_cpy, self.get_all_arcs()):
                continue
    ...
    def select_unassigned_variable(self, assignment: dict[str, list[T]]) -> str:
        ...
        def fewestLegalValues(key: str) -> int | float:
            n = len(assignment[key])
            return n if n > 1 else float("inf")

        return min(assignment.keys(), key=fewestLegalValues)

    def inference(
        self, assignment: dict[str, list[T]], queue: list[tuple[str, str]]
    ) -> bool:
    ...
    def revise(self, assignment: dict[str, list[T]], i: str, j: str) -> bool:
        ...
        for x in assignment[i]:
            arcs = [(x, y) for y in assignment[j]]

            if any(arc in arcs for arc in self.constraints[i][j]):
                continue
...
if __name__ == "__main__":
    ...
    for (difficulty, file) in boards:
        ...
        print("Backtracks", csp.backtracks)
